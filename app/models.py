...
class Bank(models.Model):
    ...
    def make_payment(self, amount, receiver_account):
        # Vérifiez que le montant de la transaction est valide
        if amount <= 0:
            raise forms.ValidationError('The amount must be greater than 0.')

        # Vérifiez que le compte bancaire du destinataire est valide
        if not receiver_account:
            raise forms.ValidationError('The receiver account is not valid.')
...
class Card(models.Model):
    ...
    def make_payment(self, amount, receiver_account):
        ...
        #     recipient=receiver_account.user,
        #     message='Vous avez reçu un paiement de {amount} de {sender}.'.format(
        #         amount=amount, sender=self.user
        #     ),
        # )
        # notification.send()
