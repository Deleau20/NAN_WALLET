...
class AccountList(LoginRequiredMixin, ListView, PermissionRequiredMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['bank'] = Bank.objects.filter(payment__user=user)
        context['card'] = Card.objects.filter(payment__user=user)
        return context
...
class SendSearchUser(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if query:
            ...
            if not search_user:
                messages.error(self.request, 'Aucun utilisateur trouvé.')
        else:
            search_user = []
...
class SendMoney(LoginRequiredMixin, CreateView, PermissionRequiredMixin):
    ...
    #     # Vérifiez que le mode de paiement sélectionné est valide
    #     try:
    #         transaction.payment_method = PaymentMethod.objects.get(id=form.cleaned_data['payment_method'], user=self.request.user)
    #     except PaymentMethod.DoesNotExist:
    #         return HttpResponseNotFound('Invalid payment method.')
    ...
    def form_valid(self, form):
        ...
        try:
            if payment_method.method_type == 'account':
                transaction.payment_method = PaymentMethod.objects.get(id=payment_method.pk, user=self.request.user)
            elif payment_method.method_type == 'bank':
                transaction.payment_method = Bank.objects.get(id=payment_method.pk)
                # Effectuez la transaction avec la banque
                transaction.payment_method.make_payment(transaction.amount, transaction.receiver.account)
            elif payment_method.method_type == 'card':
                transaction.payment_method = Card.objects.get(id=payment_method.pk)
                # Effectuez la transaction avec la carte bancaire
                transaction.payment_method.make_payment(transaction.amount, transaction.receiver.account)
            else:
                raise forms.ValidationError('Unsupported payment method.')
        except (PaymentMethod.DoesNotExist, Bank.DoesNotExist, Card.DoesNotExist):
            return HttpResponseNotFound('Invalid payment method.')
